...
exports.criarFormaPagamento = async (req, res) => {
    try {
        ...
        res.status(201).json({ sucesso: true, mensagem: 'Formma de Pagamento inserida com sucesso!', pagemento: result.rows[0] });
    } catch (error) {
...
exports.atualizarFormaPagamento = async (req, res) => {
    try {
        ...
        res.status(500).json({ sucesso: false, mensagem: 'Erro ao atualizar froma de pagamento.' });
    }
};
...
exports.deletarFormaPagamento = async (req, res) => {
    try {
        ...
        if (error.code === '23503') {
            return res.status(400).json({ sucesso: false, mensagem: 'Não é possível excluir: existem pedidos associados a essa forma de pagamento.' });
        }
